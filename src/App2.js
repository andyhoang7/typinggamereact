...
class App extends React.Component {
  ...
  async setdata() {
    const resp = await fetch (`http://localhost:5000/excerpts`)
    const data = await resp.json()
    this.setState({
      excerpts: data,
      excerpt: this._randomElement(data),
    })
    console.log("check", this.state.excerpt)
  }
  ...
  _postScore = async (wpm, errorCount) => {
    const score = {
      wpm: wpm,
      errors: errorCount,
      time: this.state.timeElapsed / 1000,
    }
    const resp = await fetch(`http://localhost:5000/postscore`, {
      method: 'POST',
  ...
  _handleInputChange = e => {
    ...
    if (this.state.excerpt.slice(index, index + inputVal.length) === inputVal) {
      if (inputVal.slice(-1) === " " && !this.state.error) {
        ...
        console.log("Loi said clg okay", this.state)
        this.setState({
          value: '',
          completed: true
        }, function () {
  ...
  _calculateWPM = () => {
    ...
    if (this.state.completed) {
      ...
      console.log('Hi Im Hang', this.state)
      this._postScore(wpm, this.state.errorCount)
    } else {
